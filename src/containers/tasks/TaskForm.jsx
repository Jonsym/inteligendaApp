    ...
    return ( 
        <div className="min-h-screen overflow-y-auto bg-white">
            ...
            <div className="max-w-screen-xl px-10 py-10 mx-auto">
                ...
                <h1 className="mb-5 text-2xl font-bold text-gray-700"> { selected && selected._id !== 0 ? 'Edit':'Crear nueva' } actividad</h1>

                <form className="w-full my-6" onSubmit={submit}>

                    <div className="relative mb-5">
                        <label className="block mb-3 text-sm font-bold" htmlFor="">Titulo</label>
                        {
                            loadingTask ? <InputLoader/> : 
                            <input disabled={loadingTask} value={title} name="title" onChange={handleChange} className="w-full h-16 px-10 py-4 field-control" type="text" placeholder="Escribe un titulo"/>
                        }
                        <span className="block px-4 text-sm text-red-400"></span>
                    </div>

                    <div className="relative mb-5">
                        <label className="block mb-3 text-sm font-bold" htmlFor="">Contenido</label>
                        {
...

                    {
                        form.project && (
                            <div className="relative max-w-md mb-5">
                                <label className="block mb-3 text-sm font-bold" htmlFor="">Seleccionar proyecto</label>
                                {
                                    loadingTask ? <InputLoader/> : 
                    ...
                    <div className="relative max-w-md mb-5">
                        <label className="block mb-3 text-sm  font-bold" htmlFor="">Tags</label>

                        {
                            loadingTask ? <InputLoader/> : 
                            <div className="flex items-center ">
                                <input maxLength="15" onChange={e => setTag(e.target.value)} value={tag} name="title" className="px-10 py-4 field-control" type="text" placeholder="Escribe un tag"/>
                    ...
                    <div className="flex justify-end p-4 text-left">
                        <button type="submit" 
                            ...
                            disabled={loadingTask}> { loadingTask && <SpinnerI/> } {loadingTask ? 'Loading':'Guardar actividad'}
                        </button>
                    </div>
