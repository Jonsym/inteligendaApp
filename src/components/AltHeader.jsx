      ...
      <div className="flex -mr-2 md:hidden">
        ...
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <Link to={HOME}>
            <div class="mx-w-10 text-3xl font-bold capitalize text-gray-900 flex items-center">Inteligenda</div>
            </Link>
          </div>
              ...
              {({open})=> (
                <>
                  ...
                  <Menu.Items className={`focus:outline-none block absolute right-0 w-48 py-1 mt-4 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5`}>
                    <Menu.Item>
                      {({active}) => (
                        <Link className={`block px-4 py-2 text-sm ${active ? 'bg-gray-100':'text-gray-700'}`} role="menuitem" to={PROFILE}>Tu perfil</Link>
                      )}
                    </Menu.Item>
                    <Menu.Item>
                      {({active}) => (
                        <Link className={`block px-4 py-2 text-sm ${active ? 'bg-gray-100':'text-gray-700'}`} role="menuitem" to={CHANGE_PASSWORD}>Cambiar contraseña</Link>
                      )}
                    </Menu.Item>
                    <Menu.Item>
                      {({active}) => (
                      <button onClick={() => Logout()} type="button" className={`block w-full px-4 py-2 text-sm text-left ${active ? 'bg-gray-100':'text-gray-700'}`} role="menuitem">Cerrar sesión</button>
                      )}
                    </Menu.Item>
                  </Menu.Items>
                  </Transition>
    ...
    <div  className={` ${toggled ? '':'hidden'} md:hidden mobile-menu`} id="mobile-menu">
      <div className="pt-4 pb-3 border-t border-gray-300">
        ...
        <div className="px-2 mt-3 space-y-1">
          <Link className="block px-3 py-2 text-base font-medium text-gray-400 rounded-md hover:text-white hover:bg-gray-700" to={PROFILE}>Tu perfil</Link>
          <Link className="block px-3 py-2 text-base font-medium text-gray-400 rounded-md hover:text-white hover:bg-gray-700" to={CHANGE_PASSWORD}>Cambiar contraseña</Link>
          <button onClick={() => Logout()} className="block px-3 py-2 text-base font-medium text-gray-400 rounded-md hover:text-white hover:bg-gray-700" type="button">Cerrar sesión</button>
        </div>
      </div>
    </div>
  </nav>
