...
const UserTasks = ({tasks, loading, title}) => {
    ...
    return ( 
        <Card>
            <Card.Header> { title || "Mis actividades" }</Card.Header>
            <Card.Body>

                <select value={filter} onChange={e => setFilter(e.target.value)} valu={filter} disabled={loading} className="field-control">
                        ...
                        !loading && !filtered.length && (
                            <div className="flex items-center justify-center border border-gray-300 border-opacity-50 border-dashed rounded">
                                <div className="w-full p-4 text-sm text-center text-gray-400">
                                    ...
                                    {
                                        filter === "all" && <div className="my-2"><Link className="button is-primary" to={ADD_TASK}>Agregar actividad</Link></div>
                                    }
                                </div>
                            </div>
                        )
