...
const TaskComments = () => {
    ...
    return ( 
        <>
            <div className="">
                <p className="mb-4 text-base text-gray-400">Comentarios</p>
                ...
                <button onClick={() => handlePostComment()} type="button" 
                    className={`px-8 py-2 text-sm font-semibold text-blue-400 bg-white border-2 border-blue-400 rounded hover:text-white hover:border-blue-500 focus:outline-none hover:bg-blue-500 ${loadingComment ? 'flex space-between justify-center border-blue-500 bg-blue-500 text-white':'block '}`}
                    disabled={loadingComment || loadingComments || editMode}>
                        { loadingComment && <SpinnerI/>}
                        {loadingComment ? '':'Guardar'}
                </button>
            </div>
